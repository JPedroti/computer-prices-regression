...
and checks whether that near-tie survives repeated cross-validation.

Protocol
--------
Repeated K-Fold (5 folds x 2 repeats, seed 42) on the development split only.
The overfitting holdout is never touched here.
"""
...
VALIDATION_STRATEGY = f"RepeatedKFold({CV_FOLDS}x{CV_REPEATS},seed={RANDOM_SEED})"
...
MODEL_GRID: list[tuple[str, dict, str | None, str | None]] = [
    ...
    ("median", {}, None, None),
    ("linear", {}, None, None),
    ("ridge", {"alpha": 1.0}, None, None),
    ("ridge", {"alpha": 10.0}, None, None),
    ("lasso", {"alpha": 0.1}, None, None),
    ("elasticnet", {"alpha": 0.1, "l1_ratio": 0.5}, None, None),
    ("tree", {"max_depth": 12, "min_samples_leaf": 50}, None, None),
    ("rf", {"n_estimators": 300, "min_samples_leaf": 5}, None, None),
    ("et", {"n_estimators": 300, "min_samples_leaf": 5}, None, None),
    ...
    # target transform probe on the strongest fast learner
...
def main() -> None:
    ...
    print(f"development rows={len(X_dev)}  features={X_dev.shape[1]}")
    ...
    for name, params, prep, ttf in MODEL_GRID:
        ...
        res = cross_validate_model(
            ...
            n_splits=CV_FOLDS, n_repeats=CV_REPEATS, seed=RANDOM_SEED, return_oof=False,
        )
        print(res.summary(), flush=True)
        tracker.log(
            model=tag,
            features=cfg.enabled(),
            preprocessing=describe(name, prep, ttf).split("|")[1],
